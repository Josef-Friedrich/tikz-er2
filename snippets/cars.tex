...
\begin{tikzpicture}[er2 colorful,every edge/.style={link},scale=0.7,transform shape]
    ...
    \node[attribute] (lic) [node distance=0.2cm,above=of car] {\key{License}} edge (car);
    ...
    \node[attribute] (pri) [node distance=0.5cm,above left=of buy,nearest] {Price} edge (buy);
    \node[attribute] (sdate) [node distance=0.5cm,left=of buy,nearest] {Date} edge (buy);
    \node[attribute] (bval) [node distance=0.5cm,below left=of buy] {Value} edge (buy);
